...
class Config:
    ...
    train_pump: bool = True # keep True in NeuroPump training
    seathru_data: bool = False # to get linear and srgb renderin gresults
    use_srgb_data: bool = True # NeuroPump deals with common sRGB inputs 
    render_with_refraction: bool = True # A switch, if True, use refracted directions to render ###**
    ...
    factor: int = 2  # The downsample factor of images, 0 for no downsampling.
    ...
    lr_init: float = 0.001  # The initial learning rate.
    lr_final: float = 0.00001  # The final learning rate.
    ...
    clip: bool=True
    grad_max_norm1: float = 0.001  # Gradient clipping magnitude, disabled if == 0.
    grad_max_norm2: float = 0.0005  # Gradient clipping magnitude, disabled if == 0.
    ...
    trans_loss_mult: float = 0.01



    ### NeuroPump figs
    uwrays: bool = True # Overide this True for underwater training with refraction, False for rendering in-air images
    use_color_cast_loss: bool = True  # use haze constraint to constraint haze for NeuroPump
    render_new_optic: str = None # New optics rendering of NeuroPump. There are 3 modes: change_background_light / change_s / change_refraction_index
    new_bgl_r: float = 0.
    new_bgl_g: float = 0.
    new_bgl_b: float = 0.
    # These color averages must be overide in bash scripts
    r_avg: float = 0.
    g_avg: float = 0.
    b_avg: float = 0.
    n_w: float = 1.333 # water refraction index
    n_a: float = 1.0 # air refraction index 
    s: float = 0.02 # distance between optical center and lens case
    step_threshold: int = 35000 # first step: 0 - 35000, second step: 35000 - 150000
    haze_mult_step0: float = 0. # the color cast loss weight in first training step
    haze_mult_step1: float = 0.01 # the color cast loss weight in second training step
    # for render
    render_scene: str = "" # data name, for render
    parent_folder: str = "" # folder to save render images

    show_uw_params_step: int = 100 # show scattering parameters every 100 step 
    show_uw_params: bool = True # show scattering parameters
