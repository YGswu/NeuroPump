  ...
  def _load_renderings(self, config):
    ...
    for _, frame in enumerate(meta['frames']):
      ...
      if self._use_tiffs:
        ...
        image = get_img('.png') / 255. # 24
        image = jnp.where(image > limit1, ((image + 0.055) / 1.055) ** 2.4, image / 12.92)
...
class LLFF(Dataset):
  ...
  def _load_renderings(self, config):
    ...
    if raw_testscene:
      ...
      poses = raw_testscene_poses[split_for_data_loading]  # TODO Naama
    ...
    if split_for_data_loading == utils.DataSplit.TRAIN:
      ...
      self.images_files = [self.images_files[i] for i in indices]
