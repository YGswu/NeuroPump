...
"""NeuroPump and its MLPs, with helper functions for construction and rendering."""
...
class AModule(nn.Module):
    ...
    def __call__(self, key):
        A = self.param('A', lambda key: jnp.array([0.9, 0.9, 0.9], dtype=jnp.float32))
        return A

class BetasModule(nn.Module):
    @nn.compact
    def __call__(self, key):
        betas = self.param('betas', lambda key: jnp.array([0.4, 0.2, 0.2], dtype=jnp.float32))
        return betas
# NeuroPump's parameters <<<
...
class Model(nn.Module):
    ...
    def __call__(
        ...
        for i_level in range(self.num_levels):
            ...
            # NeuroPump render
            rendering = render.volumetric_rendering_ours(
                A_=A,
                betas_=betas,
            ...
            if compute_extras:
                ...
                rendering['ray_xy'] = rays.imageplane[:n, :]
                rendering['ray_x'] = rays.x_coord[:n]
                rendering['ray_y'] = rays.y_coord[:n]
