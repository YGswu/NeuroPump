...
def volumetric_rendering_ours(
    ...
    def srgb2linear(x):
        limit1 = 0.04045
        l =  jnp.where(x > limit1, ((x + 0.055) / 1.055) ** 2.4, x / 12.92)
        return l

    def linear2srgb(x):
        limit2 = 0.0031308
        linear_transformed = jnp.where(x > limit2, 1.055 * jnp.power(x + 0.00001, 1.0 / 2.4) - 0.055, 12.92 * x)
           
        srgb_clipped = jnp.clip(linear_transformed, 0.0, 1.0)
    ...
    J_r = (weights[..., None] * rgbs).sum(axis=-2)
    

    rendering['t_dist'] = tdist

    A = jnp.clip(A_, 0., 1.)
    betas = jnp.clip(betas_, 0.1, 1.)
    ...
    if True:
        ...
        # get detach values
        detached_depth = jax.lax.stop_gradient(depth)
        detached_betas = jax.lax.stop_gradient(betas)
        detached_A = jax.lax.stop_gradient(A)

        # for sct optimize
        T_r = jnp.exp(-(depth * betas[..., 0]))
        T_g = jnp.exp(-(depth * betas[..., 1]))
        T_b = jnp.exp(-(depth * betas[..., 2]))

        # for stop betas optimize
        detach_T_r = jnp.exp(-(detached_depth * detached_betas[..., 0]))
        detach_T_g = jnp.exp(-(detached_depth * detached_betas[..., 1]))
        detach_T_b = jnp.exp(-(detached_depth * detached_betas[..., 2]))

        # for stop depth optimize
        detach_d_T_r = jnp.exp(-(detached_depth * betas[..., 0]))
        detach_d_T_g = jnp.exp(-(detached_depth * betas[..., 1]))
        detach_d_T_b = jnp.exp(-(detached_depth * betas[..., 2]))


        # make linear-colors for calculate
        if config.use_srgb_data:
          A = srgb2linear(A)
          J_r = srgb2linear(J_r)

        if gin.query_parameter('Config.render_new_optic') == "change_background_light":
            # for new optics parameter rendering of NeuroPump
            new_bgl_r = gin.query_parameter('Config.new_bgl_r')
            new_bgl_g = gin.query_parameter('Config.new_bgl_g')
            new_bgl_b = gin.query_parameter('Config.new_bgl_b')
            background_light = jnp.concatenate(((1.0 - T_r) * new_bgl_r, (1.0 - T_r) * new_bgl_g, (1.0 - T_r) * new_bgl_b), axis=-1)
            JT = jnp.concatenate(( (J_r[..., 0])[...,None] * T_r, (J_r[..., 1])[...,None] * T_r, (J_r[..., 2])[...,None] * T_r), axis=-1)
            I_r = JT + background_light
        else:
            # for training and rectified rendering
            background_light = jnp.concatenate(((1.0 - T_r) * A[..., 0], (1.0 - T_g) * A[..., 1], (1.0 - T_b) * A[..., 2]), axis=-1)
            JT = jnp.concatenate(( (J_r[..., 0])[...,None] * T_r, (J_r[..., 1])[...,None] * T_g, (J_r[..., 2])[...,None] * T_b), axis=-1)
            I_r = JT + background_light

        detach_background_light = jnp.concatenate(((1.0 - detach_T_r) * detached_A[..., 0], (1.0 - detach_T_g) * detached_A[..., 1], (1.0 - detach_T_b) * detached_A[..., 2]), axis=-1)
        detach_JT = jnp.concatenate(( (J_r[..., 0])[...,None] * detach_T_r, (J_r[..., 1])[...,None] * detach_T_g, (J_r[..., 2])[...,None] * detach_T_b), axis=-1)
        detach_I_r = detach_JT + detach_background_light

        # for loss calculation
        rendering['hazeconst'] = jnp.concatenate(((1.0 - detach_d_T_r) * A[..., 0], (1.0 - detach_d_T_g) * A[..., 1], (1.0 - detach_d_T_b) * A[..., 2]), axis=-1)
        rendering['betas'] = betas * jnp.ones_like(detach_d_T_r)

        # linear2srgb
        if config.use_srgb_data:
          srgb_I_r = linear2srgb(I_r)
          srgb_J_r = linear2srgb(J_r)
          srgb_background_light = linear2srgb(background_light)
          srgb_detach_I_r = linear2srgb(detach_I_r)

        rendering['rgbstpgrd'] = srgb_detach_I_r
        rendering['rgb'] = srgb_I_r
        rendering['Haze'] = srgb_background_light
        rendering['J'] = srgb_J_r
