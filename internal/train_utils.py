...
def compute_data_loss(batch, renderings, rays, config, opts_mult):
    data_losses = []
    ...
    for rendering in renderings:
        ...
        resid_sq_stp1 = (rendering['J'] - batch.rgb[..., :3]) ** 2
        resid_sq_stp2 = (rendering['rgb'] - batch.rgb[..., :3]) ** 2
        resid_sq_stp3 = (rendering['rgbstpgrd'] - batch.rgb[..., :3]) ** 2
        resid_sq = resid_sq_stp1 * opts_mult[0] + resid_sq_stp2 * opts_mult[1]
        denom = lossmult.sum()
        ...
        if config.data_loss_type == 'mse':
            ...
            rgb_render_clip = jnp.minimum(1., rendering['J'] * opts_mult[0] + rendering['rgb'] * opts_mult[1])
...
## constraints ##**
def color_cast_loss(rendering, config, haze_mult):
    """
    The color-cast loss for NeuroPump training.
    """
    ...
    last_ray = rendering[-1]
    hat = last_ray['hazeconst']
    rhat_ = hat[...,0]
    ghat_ = hat[...,1]
    bhat_ = hat[...,2]

    # Clip to avoid NaN in this calculation
    rhat_ = jnp.clip(rhat_, 0.00001, 1)
    ghat_ = jnp.clip(ghat_, 0.00001, 1)
    bhat_ = jnp.clip(bhat_, 0.00001, 1)

    ratio_rg = rhat_ / ghat_
    ratio_rb = rhat_ / bhat_
    ratio_gb = ghat_ / bhat_

    loss_rg = jnp.maximum(ratio_rg - r_avg / g_avg, 0)
    loss_rb = jnp.maximum(ratio_rb - r_avg / b_avg, 0)
    loss_gb = jnp.abs((g_avg / b_avg - ratio_gb))

    loss = jnp.mean(loss_rg + loss_rb + loss_gb)
    loss = jnp.array(loss)

    return loss * haze_mult

def smooth_relu(x, epsilon=1e-6):
    return jnp.log(1 + jnp.exp(x)) + epsilon


def uw_acc_weights_loss(rays, batch, rendering, config):
    """Computes the distortion accuracy loss regularizer defined in seaThru-NeRF on the object's weights sum."""
    last_ray_results = rendering[-1]
    weights = last_ray_results['weights'].sum(axis=-1)
    data_loss = 0.
    data_loss += jnp.mean((
        (-jnp.log(jnp.exp(-jnp.abs(1 - weights) / 0.1) + config.uw_acc_loss_factor * jnp.exp(-jnp.abs(weights) / 0.1)))))
    data_losses = jnp.array(data_loss)
...
def uw_acc_tran_loss(ray_history, renderings, batch, config):
    """Computes the distortion accuracy loss regularizer defined in seaThru-NeRF on the object's transmission."""

    last_ray_results = renderings[-1]
    weights = last_ray_results['trans']

    data_loss = 0

    data_loss += jnp.mean((
        (-jnp.log(config.uw_acc_loss_factor * jnp.exp(-jnp.abs(1 - weights) / 0.1) + jnp.exp(-jnp.abs(weights) / 0.1)))))
...
def uw_sig_med_loss(ray_history, renderings, batch, config):
    """Computes the std loss on medium densities - not in the paper ."""
    last_ray_results = renderings[-1]

    weights = last_ray_results['sigma_bs']
    weights_bs = last_ray_results['sigma_atten']

    weights_cmed = last_ray_results['c_med']

    data_loss = 0

    data_loss += jnp.mean((jnp.std(weights_bs, axis=0))) + jnp.mean((jnp.std(weights, axis=0)))

    data_losses = jnp.array(data_loss)

    loss = (
            config.uw_sig_med_mult * data_losses)
    
    return loss


def clip_gradients(grad, config, gds_mult):
    ...
    for k, g in grad['params'].items():
        ...
        if config.clip:
            mult = jnp.minimum(
                1, gds_mult / (jnp.finfo(jnp.float32).eps + tree_norm(g)))
...
def create_train_step(model: models.Model,
    ...
    def train_step(
        ...
        def loss_fn(variables):
            ...
            data_loss, stats = compute_data_loss(batch, renderings, rays, config, opts_mult)
            ...
            if config.use_color_cast_loss:
                losses['color_cast_loss'] = color_cast_loss(renderings, config, haze_mult)
        ...
        grad = clip_gradients(grad, config, gds_mult)

        #grad = jax.tree_util.tree_map(lambda x: x, grad)
...
def setup_model(
    ...
    MutableMapping[Text, Any]], Callable[
               [jnp.array, TrainState, utils.Batch, Optional[Tuple[Any, ...]], float, float, float],
