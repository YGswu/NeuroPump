...
def pixels_to_rays(
  ...
  pp_c = xnp.array([0.0, 0.0, 1.0])# camera coord [0,0,1] ##**
  
  


  ### refraction ###
  if gin.query_parameter('Config.uwrays'):

      normal = mat_vec_mul(camtoworlds[..., :3, :3], pp_c)
      normal_ = normal/(xnp.linalg.norm(normal, axis=-1, keepdims=True))

      n_a = gin.query_parameter('Config.n_a')
      n_w = gin.query_parameter('Config.n_w')

      sin_in_ = (xnp.linalg.norm(np.cross(normal_, viewdirs), axis=-1)) / (
                  (xnp.linalg.norm(normal_, axis=-1,)) * (xnp.linalg.norm(viewdirs, axis=-1, )))
      sin_in = sin_in_[...,None]
      sin_out = (n_a / n_w) * sin_in

      theta1 = xnp.arcsin(xnp.clip(sin_in, -1, 1))
      theta2 = xnp.arcsin(xnp.clip(sin_out, -1, 1))
      theta = - (theta1 - theta2)

      u_ = np.cross(viewdirs, normal_, axis=-1)
      u = u_ / xnp.linalg.norm(u_, axis=-1, keepdims=True)
      
      dot = xnp.multiply(u, viewdirs)
  
      dot = xnp.sum(dot, axis=-1)[...,None]
     

      dir_out = viewdirs * xnp.cos(theta) + xnp.cross(u, viewdirs, axis=-1) * xnp.sin(theta) + dot * u * (
                  1 - xnp.cos(theta))

      viewdirs_ref = dir_out / xnp.linalg.norm(dir_out, axis=-1, keepdims=True)

      cos_in = xnp.sqrt(1 - (sin_in) ** 2)
      cos_out = xnp.sqrt(1 - (sin_out) ** 2)
      d_s = 0.02
      if gin.query_parameter('Config.render_new_optic') == 'change_s':
        d_s = gin.query_parameter('Config.s')
      d = d_s / cos_in
      d = d * viewdirs
      f_world = xnp.linalg.norm(normal, axis=-1, keepdims=True)
      origins = origins + d

      lengths = ( xnp.linalg.norm(directions, axis=-1, keepdims=True) ) * sin_in

      directions = viewdirs_ref
      viewdirs = viewdirs_ref
  else: 
      # without water
      normal = mat_vec_mul(camtoworlds[..., :3, :3], pp_c)
      normal_ = normal/(xnp.linalg.norm(normal, axis=-1, keepdims=True))

      n_a = gin.query_parameter('Config.n_w')
      n_w = n_a
  ...
  if pixtocam_ndc is None:
    ...
    dx_norm = xnp.linalg.norm(drx - dirs, axis=-1)
    dy_norm = xnp.linalg.norm(dry - dirs, axis=-1)
