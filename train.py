...
def main(unused_argv):
    ...
    for step, batch in zip(range(init_step, num_steps + 1), pdataset):
        ...
        # if step >= config.uw_decay_acc:
            # sig_mult = config.final_acc_trans_loss_mult
        # else:
        sig_mult = config.trans_loss_mult
            

        if step < config.step_threshold:
            opts_mult = np.array([1., 0.])
            haze_mult = config.haze_mult_step0
            gds_mult = config.grad_max_norm1
        elif step >= config.step_threshold:
            opts_mult = np.array([0., 1.])
            haze_mult = config.haze_mult_step1
            gds_mult = config.grad_max_norm2
        ...
        if jax.host_id() == 0:
            ...
            if step == init_step or step % config.print_every == 0:
                ...
                str_losses = {  # Grab each "losses_{x}" field and print it as "x[:4]".
                    k[7:11]: (f'{v:0.18f}' if v >= 1e-4 and v < 10 else f'{v:0.1e}')
        ...
        if config.train_render_every > 0 and step % config.train_render_every == 0:
            ...
            if jax.host_id() == 0:
                ...
                if config.rawnerf_mode:
                    ...
                    summ_fn_wandb('test/true_auto', wandb.Image(postprocess_fn(test_case.rgb, None)), step)
                    ...
                    for p, x in list(exposures.items()):
                        ...
                        summ_fn_wandb(f'test/true_color/{p}', wandb.Image(postprocess_fn(test_case.rgb, x)), step)

                # summary_writer.image('test_true_color', test_case.rgb, step)
                summ_fn_wandb(f'test/true_color', wandb.Image(test_case.rgb), step)

                if config.compute_normal_metrics:
                    # summary_writer.image('test_true_normals',
                    #                      test_case.normals / 2. + 0.5, step)
                    summ_fn_wandb(f'test/true_normals', wandb.Image(test_case.normals / 2. + 0.5), step)
                ...
                for k, v in vis_suite.items():
                    ...
                    if k in ['sigma_bs','sigma_atten']:
                        utils.save_img_u8(
                            postprocess_fn(np.asarray(v / v.max(axis=[0, 1]))),
                            os.path.join(config.checkpoint_dir, f'{k}_{step}.png'))
                        print(f'max_{k}_{np.asarray(v).max(axis=2)}')

                    else:
                        utils.save_img_u8(np.asarray(v),os.path.join(config.checkpoint_dir, f'{k}_{step}.png'))
