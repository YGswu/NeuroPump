...
from img_process import lighten_crop
...
def create_videos(config, base_dir, out_dir, out_name, num_frames):
    ...
    depth_file = os.path.join(out_dir, f'distance_median_{idx_to_str(0)}.tiff')
    ...
    for k in ['color', 'J']:
        ...
        file_ext = 'png' if k in ['color', 'J', 'distance_median'] else 'tiff'
        idx = 0
...
def main(unused_argv):
    ...
    out_name = 'path_renders' if config.render_path else 'render'
    ...
    for idx in range(dataset.size):
        ...
        if 'J' in rendering:
            rendering['J'] = postprocess_fn(rendering['J'])
            save_fn(utils.save_img_u8, rendering['J'], path_fn(f'J_{idx_str}.png'))
        if 'bs' in rendering:
            rendering['bs'] = postprocess_fn(rendering['bs'])
            save_fn(utils.save_img_u8, rendering['bs'], path_fn(f'bs_{idx_str}.png'))
        if 'Haze' in rendering:
            rendering['Haze'] = postprocess_fn(rendering['Haze'])
            save_fn(utils.save_img_u8, rendering['Haze'], path_fn(f'haze_{idx_str}.png'))

        matplotlib.image.imsave(path_fn(f'distance_median_{idx_str}.png'), np.asarray(rendering['distance_median']),vmin=0.4, vmax=3.5, )
    ...
    time.sleep(1)
    num_files = len(glob.glob(path_fn('distance_median*.tiff')))
    time.sleep(10)

    if gin.query_parameter('Config.render_new_optic') == "no":
        lighten_crop.process_folders_in_parent_folder(parent_folder = config.parent_folder, scene = config.render_scene)
        # print(parent_folder)

    print("[COMPLATE] all images are rendered!")
